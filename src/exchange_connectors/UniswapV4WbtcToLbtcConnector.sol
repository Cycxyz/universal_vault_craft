// SPDX-License-Identifier: BUSL-1.1
pragma solidity ^0.8.28;

import {IExchangeConnector_v0} from "../interface/IExchangeConnector_v0.sol";
import {ILBTC} from "../interface/ILBTC.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {PoolKey, SwapParams} from "../types/UniswapV4Structs.sol";
import {IPoolManager} from "../interface/IPoolManager.sol";
import {IPositionManager} from "../interface/IPositionManager.sol";
import {IUnlockCallback} from "../interface/IUnlockCallback.sol";
import {BalanceDeltaLibrary} from "../libraries/BalanceDeltaLibrary.sol";

contract UniswapV4WbtcToLbtcConnector is IExchangeConnector_v0, IUnlockCallback {
    using SafeERC20 for IERC20;

    /// @notice The LBTC token contract address (Ethereum mainnet)
    ILBTC public constant lbtc = ILBTC(0x8236a87084f8B84306f72007F36F2618A5634494);
    
    /// @notice The WBTC token contract address (Ethereum mainnet)
    address public constant WBTC = 0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599;
    
    /// @notice The BTC.b token contract address (Ethereum mainnet)
    address public constant BTC_B = 0xB0F70C0bD6FD87dbEb7C10dC692a2a6106817072;
    
    IPoolManager public immutable POOL_MANAGER;
    address public immutable WBTC_TOKEN;
    address public immutable BTC_B_TOKEN;
    uint24 public immutable FEE;
    int24 public immutable TICK_SPACING;
    address public immutable HOOKS;

    error InvalidPool();
    error InvalidPositionManager();
    error InvalidTokenPair();
    error InsufficientInputAmount();
    error InvalidOutputAmount();
    error SlippageExceeded();
    error InvalidCaller();
    error InvalidSwapCallback();
    error InvalidToken();
    error ManagerLocked();
    error EthTransferFailed();

    // Swap data passed via bytes to avoid storage costs
    struct SwapData {
        address assetIn;
        address assetOut;
        address recipient; // Original caller
        uint256 amountOut;
        uint256 maxAmountIn;
        uint256 amountIn;
        uint256 minAmountOut;
        bool isExchangeOut;
    }

    constructor(
        address _poolManager,
        address _positionManager,
        bytes25 _poolId
    ) {
        require(_poolManager != address(0), InvalidPool());
        require(_positionManager != address(0), InvalidPositionManager());
        
        PoolKey memory poolKey = IPositionManager(_positionManager).poolKeys(_poolId);
        require(poolKey.currency0 < poolKey.currency1, InvalidTokenPair());
        
        // Verify pool contains WBTC and BTC.b
        require(
            (poolKey.currency0 == WBTC && poolKey.currency1 == BTC_B) ||
            (poolKey.currency0 == BTC_B && poolKey.currency1 == WBTC),
            InvalidTokenPair()
        );

        POOL_MANAGER = IPoolManager(_poolManager);
        WBTC_TOKEN = poolKey.currency0 == WBTC ? poolKey.currency0 : poolKey.currency1;
        BTC_B_TOKEN = poolKey.currency0 == WBTC ? poolKey.currency1 : poolKey.currency0;
        FEE = poolKey.fee;
        TICK_SPACING = poolKey.tickSpacing;
        HOOKS = poolKey.hooks;
    }

    function exchangeOut(address assetIn, address assetOut, uint256 amountOut, uint256 maxAmountIn)
        external
        payable
        override
        returns (uint256 amountIn)
    {
        _validateTokenPair(assetIn, assetOut);
        
        // Calculate required BTC.b amount for desired LBTC output using ratio
        uint256 rate = lbtc.ratio();
        uint256 requiredBtcB = (amountOut * 1e18 + rate - 1) / rate;
        
        require(maxAmountIn > 0, SlippageExceeded());

        // Transfer maxAmountIn WBTC from sender
        IERC20(assetIn).safeTransferFrom(msg.sender, address(this), maxAmountIn);

        // Step 1: Swap WBTC -> BTC.b using Uniswap V4 (exact output)
        // We need to swap enough WBTC to get requiredBtcB
        // For exact output, we'll use a binary search or estimate
        // For simplicity, we'll swap all WBTC and check if we got enough
        uint256 btcBReceived = _swapWbtcToBtcBExactOut(requiredBtcB, maxAmountIn);
        
        require(btcBReceived >= requiredBtcB, SlippageExceeded());

        // Step 2: Deposit BTC.b -> LBTC
        SafeERC20.forceApprove(IERC20(BTC_B), address(lbtc), btcBReceived);
        lbtc.deposit(btcBReceived);

        // Calculate actual WBTC spent
        amountIn = maxAmountIn - IERC20(WBTC).balanceOf(address(this));

        // Refund excess WBTC if any
        uint256 remainingWbtc = IERC20(WBTC).balanceOf(address(this));
        if (remainingWbtc > 0) {
            IERC20(WBTC).safeTransfer(msg.sender, remainingWbtc);
        }

        // Post-checks: ensure no tokens left in connector
        require(IERC20(WBTC).balanceOf(address(this)) == 0, "No WBTC should remain");
        require(IERC20(BTC_B).balanceOf(address(this)) == 0, "No BTC.b should remain");
        require(IERC20(assetOut).balanceOf(address(this)) == 0, "No LBTC should remain");

        return amountIn;
    }

    function exchangeIn(address assetIn, address assetOut, uint256 amountIn, uint256 minAmountOut)
        external
        payable
        override
        returns (uint256 amountOut)
    {
        _validateTokenPair(assetIn, assetOut);

        // Transfer WBTC from sender
        IERC20(assetIn).safeTransferFrom(msg.sender, address(this), amountIn);

        // Step 1: Swap WBTC -> BTC.b using Uniswap V4
        uint256 minBtcBOut = 0; // Accept any amount for now
        uint256 btcBReceived = _swapWbtcToBtcB(amountIn, minBtcBOut);

        // Step 2: Calculate expected LBTC amount using ratio
        uint256 rate = lbtc.ratio();
        amountOut = (btcBReceived * rate) / 1e18;

        require(amountOut >= minAmountOut, SlippageExceeded());

        // Step 3: Deposit BTC.b -> LBTC
        SafeERC20.forceApprove(IERC20(BTC_B), address(lbtc), btcBReceived);
        lbtc.deposit(btcBReceived);

        // Post-checks: ensure no tokens left in connector
        require(IERC20(WBTC).balanceOf(address(this)) == 0, "No WBTC should remain");
        require(IERC20(BTC_B).balanceOf(address(this)) == 0, "No BTC.b should remain");
        require(IERC20(assetOut).balanceOf(address(this)) == 0, "No LBTC should remain");
    }

    function _swapWbtcToBtcB(uint256 amountIn, uint256 minAmountOut) internal returns (uint256 amountOut) {
        // Determine swap direction: zeroForOne = true if swapping currency0 -> currency1
        // Pool keys are sorted: currency0 < currency1
        // WBTC (0x2260...) < BTC_B (0xB0F7...), so WBTC is currency0, BTC_B is currency1
        bool zeroForOne = WBTC == WBTC_TOKEN; // true if WBTC is currency0
        
        // Prepare swap data
        SwapData memory swapData = SwapData({
            assetIn: WBTC,
            assetOut: BTC_B,
            recipient: address(this), // Receive BTC.b in this contract
            amountOut: 0,
            maxAmountIn: 0,
            amountIn: amountIn,
            minAmountOut: minAmountOut,
            isExchangeOut: false
        });

        // Sync currency before swap
        POOL_MANAGER.sync(WBTC);

        // For zeroForOne: price decreases, need a minimum price
        // Current price appears to be below 2^96, so use a smaller value
        // Try 2^80 = 1208925819614629174706176 as a reasonable lower bound
        // For oneForZero: price increases, use max to allow any increase
        uint160 sqrtPriceLimitX96 = zeroForOne ? uint160(1208925819614629174706176) : type(uint160).max;

        // Execute swap via unlock mechanism
        SwapParams memory params = SwapParams({
            zeroForOne: zeroForOne,
            amountSpecified: -int256(amountIn), // Negative for exact input
            sqrtPriceLimitX96: sqrtPriceLimitX96
        });

        // Encode both SwapParams and SwapData to pass via bytes
        bytes memory unlockData = abi.encode(params, swapData);
        bytes memory result = POOL_MANAGER.unlock(unlockData);

        // Decode the returned amountOut
        amountOut = abi.decode(result, (uint256));
        require(amountOut >= minAmountOut, SlippageExceeded());
    }

    function _swapWbtcToBtcBExactOut(uint256 amountOut, uint256 maxAmountIn) internal returns (uint256 amountOutReceived) {
        // Determine swap direction: zeroForOne = true if swapping currency0 -> currency1
        bool zeroForOne = WBTC == WBTC_TOKEN; // true if WBTC is currency0
        
        // Prepare swap data for exact output
        SwapData memory swapData = SwapData({
            assetIn: WBTC,
            assetOut: BTC_B,
            recipient: address(this), // Receive BTC.b in this contract
            amountOut: amountOut,
            maxAmountIn: maxAmountIn,
            amountIn: 0,
            minAmountOut: 0,
            isExchangeOut: true
        });

        // Sync currency before swap
        POOL_MANAGER.sync(WBTC);

        // For zeroForOne: price decreases, need a minimum price
        // Current price appears to be below 2^96, so use a smaller value
        // Try 2^80 = 1208925819614629174706176 as a reasonable lower bound
        // For oneForZero: price increases, use max to allow any increase
        uint160 sqrtPriceLimitX96 = zeroForOne ? uint160(1208925819614629174706176) : type(uint160).max;

        // Execute swap via unlock mechanism (exact output)
        SwapParams memory params = SwapParams({
            zeroForOne: zeroForOne,
            amountSpecified: int256(amountOut), // Positive for exact output
            sqrtPriceLimitX96: sqrtPriceLimitX96
        });

        // Encode both SwapParams and SwapData to pass via bytes
        bytes memory unlockData = abi.encode(params, swapData);
        bytes memory result = POOL_MANAGER.unlock(unlockData);

        // Decode the returned amountIn
        uint256 amountIn = abi.decode(result, (uint256));
        require(amountIn <= maxAmountIn, SlippageExceeded());
        
        amountOutReceived = amountOut;
    }

    function unlockCallback(bytes calldata data) external override returns (bytes memory) {
        require(msg.sender == address(POOL_MANAGER), InvalidCaller());

        (SwapParams memory params, SwapData memory swapData) = abi.decode(data, (SwapParams, SwapData));

        // Execute swap and extract amounts
        int256 swapDelta = POOL_MANAGER.swap(_getPoolKey(), params, "");
        (uint256 amountIn, uint256 amountOut) = _extractSwapAmounts(swapDelta, params.zeroForOne);

        // Validate and process swap
        if (swapData.isExchangeOut) {
            // Exact output swap
            require(amountOut == swapData.amountOut, InvalidOutputAmount());
            require(amountIn <= swapData.maxAmountIn, SlippageExceeded());
            
            // Transfer output (BTC.b) to this contract
            _transferOutput(swapData.assetOut, address(this), amountOut);
            
            // Settle input (WBTC)
            _settleInput(swapData.assetIn, amountIn);
            
            return abi.encode(amountIn);
        } else {
            // Exact input swap
            require(amountIn <= swapData.amountIn, InsufficientInputAmount());
            require(amountOut >= swapData.minAmountOut, SlippageExceeded());

            // Transfer output (BTC.b) to this contract
            _transferOutput(swapData.assetOut, address(this), amountOut);
            
            // Settle input (WBTC)
            _settleInput(swapData.assetIn, amountIn);

            return abi.encode(amountOut);
        }
    }

    function _extractSwapAmounts(int256 swapDelta, bool zeroForOne)
        internal
        pure
        returns (uint256 amountIn, uint256 amountOut)
    {
        int128 amount0Delta = BalanceDeltaLibrary.amount0(swapDelta);
        int128 amount1Delta = BalanceDeltaLibrary.amount1(swapDelta);

        // Negative delta = we pay (input), Positive delta = we receive (output)
        int128 amountInDelta = zeroForOne ? amount0Delta : amount1Delta;
        int128 amountOutDelta = zeroForOne ? amount1Delta : amount0Delta;

        require(amountInDelta < 0, InsufficientInputAmount());
        require(amountOutDelta > 0, InvalidOutputAmount());

        amountIn = uint256(uint128(-amountInDelta));
        amountOut = uint256(uint128(amountOutDelta));
    }

    function _transferOutput(address currency, address recipient, uint256 amount) internal {
        POOL_MANAGER.take(currency, address(this), amount);

        if (currency == address(0)) {
            (bool success,) = recipient.call{value: amount}("");
            require(success, EthTransferFailed());
        } else {
            IERC20(currency).safeTransfer(recipient, amount);
        }
    }

    function _settleInput(address currency, uint256 amount) internal {
        if (currency == address(0)) {
            POOL_MANAGER.settle{value: amount}();
        } else {
            // Transfer tokens to PoolManager first, then settle
            IERC20(currency).safeTransfer(address(POOL_MANAGER), amount);
            POOL_MANAGER.settle();
        }
    }

    function _validateTokenPair(address assetIn, address assetOut) internal pure {
        require(assetIn == WBTC && assetOut == address(0x8236a87084f8B84306f72007F36F2618A5634494), InvalidTokenPair());
    }

    function _getPoolKey() internal view returns (PoolKey memory) {
        return PoolKey({
            currency0: WBTC_TOKEN,
            currency1: BTC_B_TOKEN,
            fee: FEE,
            tickSpacing: TICK_SPACING,
            hooks: HOOKS
        });
    }

    receive() external payable {}
}
